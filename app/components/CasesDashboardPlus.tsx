...
import { Area, Bar, BarChart, CartesianGrid, Cell, ComposedChart, LabelList, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
...
const MONTHLY_TRIAGE_LEVELS = [...TRIAGE_LEVELS].reverse();
...
function getProjectId(record: EpcrRecord) {
  return record.projectInfo?.projectId || record.projectInfo?.id || record.projectId || "";
}
...
export default function CasesDashboardPlusPage({ projectId, clientMode = false }: CasesDashboardPlusProps = {}) {
  const { user, loading: userLoading } = useCurrentUser();
  const [records, setRecords] = useState<EpcrRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [clientProjectIds, setClientProjectIds] = useState<string[]>([]);
  const [clientProjectsLoading, setClientProjectsLoading] = useState(clientMode);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      projectsQuery,
      (snapshot) => {
        setClientProjectIds(snapshot.docs.map((item) => item.id));
        setClientProjectsLoading(false);
      },
      (error) => {
        console.error("Client dashboard projects listener error:", error);
        setClientProjectIds([]);
        setClientProjectsLoading(false);
      }
    );
    ...
    return unsubscribe;
  }, [clientMode, user?.uid, userLoading]);

  const scopedRecords = useMemo(() => {
    if (projectId) return records.filter((record) => getProjectId(record) === projectId);
    if (clientMode) {
      const allowedProjects = new Set(clientProjectIds);
      return records.filter((record) => allowedProjects.has(getProjectId(record)));
    }
    return records;
  }, [clientMode, clientProjectIds, projectId, records]);
  ...
  const genderRecords = useMemo(
    () =>
      projectRecords.filter((record) => {
        ...
        const healthMatches = !selectedHealth || record.patientInfo?.healthClassification === selectedHealth;
        const complaintMatches = !selectedComplaint || (record.patientInfo?.chiefComplaints || []).includes(selectedComplaint);
        return triageMatches && healthMatches && complaintMatches;
      }),
    [projectRecords, selectedTriage, selectedHealth, selectedComplaint]
  );
  ...
  const triageRecords = useMemo(
    () =>
      projectRecords.filter((record) => {
        ...
        const healthMatches = !selectedHealth || record.patientInfo?.healthClassification === selectedHealth;
        const complaintMatches = !selectedComplaint || (record.patientInfo?.chiefComplaints || []).includes(selectedComplaint);
        return genderMatches && healthMatches && complaintMatches;
      }),
    [projectRecords, selectedGender, selectedHealth, selectedComplaint]
  );
  ...
  const healthRecords = useMemo(
    () =>
      projectRecords.filter((record) => {
        ...
        const triageMatches = !selectedTriage || normalizeTriage(record.patientInfo?.triageColor) === selectedTriage;
        const complaintMatches = !selectedComplaint || (record.patientInfo?.chiefComplaints || []).includes(selectedComplaint);
        return genderMatches && triageMatches && complaintMatches;
      }),
    [projectRecords, selectedGender, selectedTriage, selectedComplaint]
  );
  ...
  const complaintRecords = useMemo(
    () =>
      projectRecords.filter((record) => {
        ...
        const triageMatches = !selectedTriage || normalizeTriage(record.patientInfo?.triageColor) === selectedTriage;
        const healthMatches = !selectedHealth || record.patientInfo?.healthClassification === selectedHealth;
        return genderMatches && triageMatches && healthMatches;
      }),
    [projectRecords, selectedGender, selectedTriage, selectedHealth]
  );
  ...
  const filteredRecords = useMemo(
    () =>
      projectRecords.filter((record) => {
        ...
        const healthMatches = !selectedHealth || record.patientInfo?.healthClassification === selectedHealth;
        const complaintMatches = !selectedComplaint || (record.patientInfo?.chiefComplaints || []).includes(selectedComplaint);
        return genderMatches && triageMatches && healthMatches && complaintMatches;
      }),
    [projectRecords, selectedGender, selectedTriage, selectedHealth, selectedComplaint]
  );
  ...
  const monthlyTriageData = useMemo(() => {
    const now = new Date();
    const months = Array.from({ length: 3 }, (_, index) => {
      ...
      return {
        ...
        ...Object.fromEntries(TRIAGE_LEVELS.map((level) => [level.key, 0])),
      } as { key: string; month: string } & Record<TriageKey, number>;
    });
    ...
    triageRecords.forEach((record) => {
      ...
      if (month) month[triage] += 1;
    });

    return months.map((month) => ({
      ...month,
      total: TRIAGE_LEVELS.reduce((sum, level) => sum + month[level.key], 0),
    }));
  }, [triageRecords]);

  const monthlyTriageDisplayData = useMemo(() => {
    if (!selectedTriage) return monthlyTriageData;

    return monthlyTriageData.map((month) => ({
      ...month,
      ...Object.fromEntries(
        TRIAGE_LEVELS.map((level) => [
          level.key,
          level.key === selectedTriage ? month[level.key] : 0,
        ])
      ),
      total: month[selectedTriage],
    })) as typeof monthlyTriageData;
  }, [monthlyTriageData, selectedTriage]);
  ...
  if (loading || userLoading || clientProjectsLoading) {
    return (
      <div className="p-6 text-sm font-semibold text-[#607482]">
        Loading Cases Dashboard...
      </div>
    );
  }

  return (
    <PermissionGuard module={clientMode ? "client_dashboards" : "dashboards"} action="epcr" showMessage={true}>
      <div className="min-h-screen bg-[#f5f7f8] p-4 text-[#274C5A] md:p-6">
        <div className="space-y-5">
          <div className="rounded-2xl bg-[#274C5A] px-5 py-4 text-white shadow-sm shadow-[#274C5A]/20">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <h1 className="text-2xl font-black tracking-tight">Cases Dashboard</h1>
            ...
            <div className="mt-4 grid grid-cols-2 border-t border-white/15 pt-3 md:grid-cols-3 xl:grid-cols-6">
              <HeaderMetric title="Total ePCR" value={`${filteredRecords.length} cases`} />
              <HeaderMetric title="Avg Response" value={`${analytics.avgResponse} min`} />
              <HeaderMetric title="Top Project" value={topProject ? `${topProject.name} · ${topProject.value} cases` : "—"} onClick={topProject ? () => setSelectedProject(topProject.name) : undefined} />
              <HeaderMetric title="Top Complaint" value={topComplaint ? `${topComplaint.name} · ${topComplaint.value}` : "—"} onClick={topComplaint ? () => setSelectedComplaint(topComplaint.name) : undefined} />
              <HeaderMetric title="Top Triage" value={topTriage?.value ? `${topTriage.label} · ${topTriage.value}` : "—"} onClick={topTriage?.value ? () => setSelectedTriage(topTriage.key) : undefined} />
              <HeaderMetric title="Projects" value={`${analytics.projects.length} active projects`} />
            </div>
          </div>

          <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
            <section className="order-5 h-[352px] overflow-hidden rounded-2xl border border-[#d8e6ea] bg-white p-4 shadow-sm">
              <div>
            ...
            <AnalyticsCard title="Top Projects by ePCR Volume" className="order-6">
            ...
            <section className="order-3 h-[352px] overflow-hidden rounded-2xl border border-[#d8e6ea] bg-white p-4 shadow-sm">
              <div>
                <div>
                  <h2 className="text-xl font-black text-[#274C5A]">
                ...
                <div className="mx-auto mt-5 max-w-[520px]">
                    <div className="grid items-center gap-3 sm:grid-cols-[1fr_150px]">
                      <svg viewBox="0 0 360 360" className="mx-auto w-full max-w-[255px] overflow-visible" role="img" aria-label="Radial triage level analysis">
                        ...
                        <text x="180" y="174" textAnchor="middle" fill="white" fontSize="11" fontWeight="700">{selectedTriage ? "SELECTED" : "TOTAL"}</text>
                        <text x="180" y="196" textAnchor="middle" fill="white" fontSize="22" fontWeight="900">{selectedTriage ? triageCounts[selectedTriage] : triageRecords.length}</text>
                      </svg>
            ...
            <AnalyticsCard title="Health Classification Analysis" className="order-4">
            ...
            <AnalyticsCard title="Chief Complaints Analysis" className="order-1">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <AnalyticsCard title="Monthly Triage Trend" className="order-2">
              <div className="flex h-full flex-col">
                <p className="mb-1 text-xs font-medium text-[#607482]">
                  Latest three months based on the ePCR reporting date.
                </p>
                <div className="min-h-0 flex-1">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={monthlyTriageDisplayData} margin={{ top: 18, right: 4, left: -20, bottom: -4 }} barCategoryGap="26%">
                      <CartesianGrid vertical={false} strokeDasharray="4 4" stroke="#e6eef1" />
                      <XAxis dataKey="month" axisLine={false} tickLine={false} stroke="#607482" />
                      <YAxis allowDecimals={false} axisLine={false} tickLine={false} stroke="#8aa0a9" />
                      <Tooltip contentStyle={tooltipStyle} />
                      {MONTHLY_TRIAGE_LEVELS.map((level) => (
                        <Area
                          key={`flow-${level.key}`}
                          type="bumpX"
                          dataKey={level.key}
                          ...
                          stackId="triage-flow"
                          fill={level.color}
                          fillOpacity={selectedTriage && selectedTriage !== level.key ? 0 : 0.1}
                          stroke={level.color}
                          strokeOpacity={selectedTriage && selectedTriage !== level.key ? 0 : 0.22}
                          strokeWidth={1}
                          isAnimationActive={false}
                          activeDot={false}
                        />
                      ))}
                      {MONTHLY_TRIAGE_LEVELS.map((level, levelIndex) => (
                        <Bar
                          key={level.key}
                          dataKey={level.key}
                          name={level.shortLabel}
                          ...
                          fill={level.color}
                          maxBarSize={78}
                          radius={levelIndex === MONTHLY_TRIAGE_LEVELS.length - 1 ? [8, 8, 0, 0] : undefined}
                          className="cursor-pointer"
                          onClick={() => setSelectedTriage((current) => current === level.key ? null : level.key)}
                        >
                          {monthlyTriageDisplayData.map((month) => (
                            <Cell
                              key={`${month.key}-${level.key}`}
                              fill={level.color}
                              opacity={selectedTriage && selectedTriage !== level.key ? 0.16 : 1}
                            />
                          ))}
                          {levelIndex === MONTHLY_TRIAGE_LEVELS.length - 1 && <LabelList dataKey="total" position="top" fill="#274C5A" fontSize={12} fontWeight={800} />}
                        </Bar>
                      ))}
                    </ComposedChart>
                  </ResponsiveContainer>
                ...
                <div className="mt-1 flex flex-wrap justify-center gap-x-3 gap-y-1">
                  {TRIAGE_LEVELS.map((level) => (
                    <button
                      key={level.key}
                      type="button"
                      className={`flex items-center gap-1 text-[11px] font-bold transition ${selectedTriage && selectedTriage !== level.key ? "opacity-25 grayscale" : "text-[#607482]"}`}
                      onClick={() => setSelectedTriage((current) => current === level.key ? null : level.key)}
                    >
...
function HeaderMetric({ title, value, onClick }: { title: string; value: number | string; onClick?: () => void }) {
  const content = (
    <>
      <span className="block text-[11px] font-semibold text-white/60">{title}</span>
      <span className="mt-1 block truncate text-sm font-black text-white">{value}</span>
    </>
  );

  return onClick ? (
    <button type="button" className="min-w-0 border-r border-white/10 px-3 py-2 text-left transition hover:bg-white/10" onClick={onClick}>
      {content}
    </button>
  ) : (
...
function AnalyticsCard({ title, children, className = "" }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <section className={`h-[352px] overflow-hidden rounded-2xl border border-[#86A7B2]/25 bg-white p-5 shadow-sm shadow-[#274C5A]/5 ${className}`}>
